...
범위: 핵심 파이프라인(①~⑥ 에이전트 + WizardState + Guardrail) + 시맨틱 캐시(`cache.py`)를 구현.
E2E·LLM-as-Judge 평가 / LangSmith류 외부 트레이싱 대시보드는 향후 확장으로 남겨둔다(`trace` 필드와
`observability.py`의 JSONL 로그가 그 위에서 평가를 돌릴 수 있는 최소 원자료를 제공한다).
...
| `cache.py` | `SearchCache`(hybrid_search 정확 일치 캐시) + `FaqCache`(FAQ 임베딩 유사도 캐시). 둘 다 `OUTPUT_DIR`에 JSON으로 write-through 저장 (아래 "중간 저장" 참고) |
| `chat_utils.py` | `last_human_text(state)` — 메시지 목록에서 가장 최근 사용자 발화만 정확히 찾는 공유 헬퍼 |
| `coordinator.py` | ① 진입 노드. 규칙 기반 우선 + 애매하면 LLM 폴백으로 `faq`/`design`/`out_of_scope` 3분기 |
| `faq_agent.py` | ② bounded ReAct(≤2회) → "정의+사용 상황+출처" 포맷 응답 |
| `wizard_supervisor.py` | ③ stage 커서 관리 + `interrupt()`로 사용자 선택 대기 (human-in-the-loop) |
| `research_worker.py` | ④ bounded ReAct(≤3회)로 위저드 각 턴의 근거 수집, 항상 Supervisor로 복귀 |
| `presenter.py` | ⑤ stage별 Pydantic structured output (기술 체크박스 표 / 파이프라인 2개 / 비교표) |
| `guardrail.py` | ⑥ 최종 출력 검증, 실패 시 1회 재시도 후 정직한 대체 응답 |
...
FAQ 경로는 `faq_agent`로, 위저드 경로는 `research_worker`로 1회만 재시도한다(`guardrail_retries` 카운터).
...
| 4 | 출력 내용 가드 | evidence에 없는 확정 표현/할루시네이션 | `guardrail.py` (LLM 검증 + 1회 재시도) |
...
4. **캐시 (`cache.py`, JSON, write-through)**
   ...
   - 트레이드오프: 두 캐시 모두 무효화(invalidation) 로직이 없다 — PDF 자료가 갱신돼도 과거 캐시가 그대로
     남는다. course-project 범위에서는 감수하고, 필요해지면 `OUTPUT_DIR`의 인덱스 갱신 시점에 캐시 파일을
     같이 지우는 방식으로 확장하면 된다.
...
- 설계 관련 질문 → 1턴(기술 선택) → `interrupt()` → 2턴(파이프라인+예/아니오) → 3턴(비교표) 순서로 진행
- 범위 밖 질문 → Coordinator가 검색 없이 즉시 안내 후 종료
- PDF에 없는 내용 질문 → `react_loop`가 `budget_exhausted`/`no_progress`로 조기 종료하고 "확인 불가"로 응답,
  `traces/{thread_id}.jsonl`에 정지 사유가 기록되는지 확인
...
- E2E/LLM-as-Judge 평가 하네스, LangSmith 등 외부 트레이싱은 미구현 (설계안 6번 우선순위와 동일)
- 캐시(`cache.py`)는 무효화 로직 없음 — 위 "중간 저장" 절 참고
