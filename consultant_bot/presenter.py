...
PROMPTS = {
    ...
    "pipeline_select": (
        ...
        "구성 판단을 스스로 추론해서 만드세요(근거 문장을 그대로 옮기는 것으로 끝내지 마세요). "
        "각 파이프라인의 steps는 반드시 '사용자 요청 1건이 시스템에 들어와 응답이 나가기까지의 런타임 처리 "
        "순서'로 쓰세요(예: '질문 입력 -> 검색 -> 근거로 답변 생성'). 모델을 어떻게 준비·학습·배포하는지 같은 "
        "구축/설치 절차는 파이프라인이 아니므로 steps에 넣지 마세요. 다만 각 기술 "
