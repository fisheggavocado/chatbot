...
| 3 | 출력 스키마 가드 | `presenter.py:69` | 파싱 불가능한 형태의 응답 → `get_structured_llm`(Pydantic `with_structured_output`) 강제 |
...
- **2026-07-21**: gcube가 느려 로컬(Windows, CPU)에서 `app.py`로 "mcp가 뭐야"/"rag가 뭐야"를 직접 실행해
  검증하던 중, "rag가 뭐야"만 좋은 근거를 찾고도 guardrail에서 두 번 다 막혀 `OUT_OF_CONTEXT_MESSAGE`로
  빠지는 실제 버그를 발견 — 검색과 생성된 답변 내용 자체는 정확했는데, `guardrail.py`의 `_rule_verdict()`가
  evidence 출처 문자열을 그대로 찾는 방식이라 LLM이 인용 시 `4-3. [강의자료] RAG (검색 증강 생성) 기초.pdf`의
  앞 번호·대괄호 라벨 접두어를 생략하고 `RAG (검색 증강 생성) 기초.pdf`만 쓰면 "근거 밖 출처를 지어냄"으로
  오판했다. 이후 같은 원인으로 "langchain이 뭐야"도 간헐적으로(LLM 호출마다 인용 형식이 달라져) 같은 방식으로
  실패하는 것을 재현 확인.
  - `guardrail.py`: `_strip_filename_prefixes()` 추가 — 원본 출처 문자열이 그대로 없어도 앞의 번호(`7-5. `)·
    대괄호 라벨(`[강의자료] `) 접두어를 벗긴 핵심 파일명으로 인용했으면 정상 인용으로 인정하도록 `_rule_verdict()`
    매칭을 완화(짧은 core 문자열은 접두어가 있던 출처에만 존재해 다른 문맥을 잘못 지우는 오탐 위험은 낮음).
  - `faq_agent.py`: `ANSWER_PROMPT`에 "파일명은 근거에 적힌 그대로 한 글자도 빠짐없이 쓰라"는 지시를 추가해
    생성 단계에서도 같은 문제를 예방(벨트-앤-서스펜더).
  - 수정 후 "rag가 뭐야"는 1차 시도에서 바로 통과·정상 답변, "langchain이 뭐야"도 재현 검증에서 통과 확인.
    같은 매칭 로직을 `lecture_agent.py`(B형)·`presenter.py`(위저드) 경로와 공유하므로 세 경로 모두 이 완화의
    혜택을 받는다.
  - 별개로 "파이프라인이 뭐야"(2026-07-21 앞선 로그 항목의 미해결 이슈)에 대해 `react_loop.py`의
    `RULE_SUFFICIENT_SCORE`를 0.5 → 0.05까지 낮춰 실측했으나 결과는 동일하게 실패 — 1차 검색 상위 2개 점수가
    [0.058, 0.027]로 `RULE_MIN_MATCHES=2` 조건(상위 2개 모두 임계 이상)을 여전히 못 채워 `reflect_llm`이
    재질의했고, 2차 검색은 오히려 더 낮은 점수([0.005, ...])만 나와 `budget_exhausted`로 종료됨. 임계값을
    아무리 낮춰도 고쳐지지 않는, 순수한 자료 내용 격차 문제임을 재확인(이 실험은 커밋하지 않고 원래 값 0.5로
    되돌림).
...
- "파이프라인이 뭐야" 같은 질문은 인코딩 손상 복구(2026-07-21) 이후에도 검색 최고 점수가 낮다(~0.058,
  ...
  (재현/추론이 필요한 케이스)로 보이며 아직 미해결. `RULE_SUFFICIENT_SCORE`를 0.05까지 낮춰 실측해봐도
  1·2차 검색 모두 상위 2개 점수가 그보다도 낮게 나와(2026-07-21) 임계값 조정으로는 해결되지 않음을 확인함 —
  근본 해결은 임계값이 아니라 자료 보강이나 질의 확장(예: "파이프라인"과 자료 표현 사이를 잇는 동의어 매핑)
  쪽에서 찾아야 한다.
