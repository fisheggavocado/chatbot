...
- 본 실행에서 확인된 이슈와 대응 (2026-07-20):
  ...
  - 기존 `HF_REPO_ID`(개인 repo `RogersHun/lecture_pdf`)가 읽기 전용이라 업로드가 `403 Forbidden`으로 막힘을
    발견 → `yeardream-toy-project/lecture_pdf`(PDF 40개 이미 업로드됨, `embedding/`은 아직 비어있음)로
    `HF_REPO_ID` 교체. 한때 읽기/쓰기 repo를 분리(`HF_TOKEN_ORG`/`HF_REPO_ID_ORG`)했다가 단일
    `HF_TOKEN`/`HF_REPO_ID`로 원복했으나, **새 repo도 같은 토큰으로 업로드 테스트했더니 여전히
    `403 Forbidden`**(PR 없이 direct push 권한 없음) — 아직 미해결. HF 계정에서 이 repo에 대한 write
    권한을 확인/부여받아야 함
- 다음 단계: HF write 권한 문제 해결 → gcube에서 `yeardream-toy-project/lecture_pdf` 대상 본 실행 → 완주 확인
  → HF `embedding/` 결과 검증(`verify_embeddings.py`) → `consultant_bot/`에서 실제 대화로 확인.
