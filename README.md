...
- 본 실행에서 확인된 이슈와 대응 (2026-07-20):
  ...
  - 기존 `HF_REPO_ID`(개인 repo `RogersHun/lecture_pdf`)가 읽기 전용이라 업로드가 `403 Forbidden`으로 막힘을
    ...
    `HF_TOKEN`/`HF_REPO_ID`로 원복. 처음 바꾼 `HF_TOKEN`으로도 이 repo에 업로드 테스트했더니 여전히
    `403 Forbidden`이 나서, write 권한이 있는 새 토큰으로 다시 교체 → 업로드/삭제 왕복 테스트 성공 확인 (2026-07-20)
- 다음 단계: gcube에서 `yeardream-toy-project/lecture_pdf` 대상 본 실행 → 완주 확인 → HF `embedding/` 결과
  검증(`verify_embeddings.py`) → `consultant_bot/`에서 실제 대화로 확인.
